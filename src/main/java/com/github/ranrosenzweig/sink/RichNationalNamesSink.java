...
public class RichNationalNamesSink extends RichSinkFunction<NationalNames> {
  ...
  public void open(Configuration parameters) throws Exception {
    ...
    String sql = "INSERT INTO national_names (id, name,year,gender,count) values (?, ?, ?, ?, ?) ";
    statement = connection.prepareStatement(sql);
    super.open(parameters);
  }

  @Override
  public void invoke(NationalNames aNationalName) {

    int BATCH_COUNT = Integer.valueOf(appProps.getProperty(AppConfig.BATCH_CONFIG));

    try {
      statement.setInt(1, aNationalName.getId());
      statement.setString(2, aNationalName.getName());
      statement.setInt(3, aNationalName.getYear());
      statement.setString(4, aNationalName.getGender());
      statement.setInt(5, aNationalName.getCount());
      statement.addBatch();
